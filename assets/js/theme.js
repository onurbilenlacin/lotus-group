...
var getColor = function getColor(name) {
    ...
    return getComputedStyle(dom)
        .getPropertyValue("--reign-".concat(name))
        .trim();
};
...
var dropdownOnHover = function dropdownOnHover() {
    var navbarArea = document.querySelectorAll(
        '[data-bs-toggle="dropdown-on-hover"]'
    );
...
var hoverDirInit = function hoverDirInit() {
    var hoverdir = document.querySelectorAll(
...
var navbarInit = function navbarInit() {
    ...
    if (navbar) {
        ...
        var handleAlpha = function handleAlpha() {
            ...
            headerOverlay.style.backgroundColor = "rgba(0, 0, 0, ".concat(
                alpha,
                ")"
            );
            navbar.style.borderColor = "rgba(255, 255, 255, ".concat(
                0.15 - alpha,
                ")"
...
var rellaxInit = function rellaxInit() {
    return (
        window.Rellax &&
        new window.Rellax("[data-parallax]", {
            speed: -3
        })
    );
};
...
var testimonialSwiperInit = function testimonialSwiperInit() {
    ...
    if (testimonials.length) {
        testimonials.forEach(function (testimonial) {
            var testimonialSlider = testimonial.querySelector(
                ".testimonial-slider"
            );

            if (testimonialSlider) {
                var testimonialAvatar = testimonial.querySelector(
                    ".testimonial-avatar"
                );
...
var thumbnailgridInit = function thumbnailgridInit() {
    var thumbnailGridContainer = document.querySelectorAll(
        ".thumbnail-grid-container"
    );

    if (thumbnailGridContainer.length) {
        thumbnailGridContainer.forEach(function (thumbnailGrid) {
            var thumbnailGridContents = thumbnailGrid.querySelectorAll(
         ...
         -----------------------------------------------*/
            ...
            attributes.forEach(function (attribute) {
                if (
                    ...
                    if (
                        utils.breakpoints[breakPointName] < currentBreakpointVal
                    ) {
